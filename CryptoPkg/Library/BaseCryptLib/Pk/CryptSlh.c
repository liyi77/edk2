  ...
  )
{
  switch (OpensslNid) {
    ...
    case EVP_PKEY_SLH_DSA_SHAKE_128F:
      *KeySize = IsPrivate? 64:32;
      break;
    case EVP_PKEY_SLH_DSA_SHA2_192S:
    case EVP_PKEY_SLH_DSA_SHA2_192F:
    case EVP_PKEY_SLH_DSA_SHAKE_192S:
    case EVP_PKEY_SLH_DSA_SHAKE_192F:
      *KeySize = IsPrivate? 96:48;
      break;
    case EVP_PKEY_SLH_DSA_SHA2_256S:
    case EVP_PKEY_SLH_DSA_SHA2_256F:
    case EVP_PKEY_SLH_DSA_SHAKE_256S:
    case EVP_PKEY_SLH_DSA_SHAKE_256F:
      *KeySize = IsPrivate? 128:64;
      break;
    default:
      return FALSE;
  }
...
/**
  ...
  @return     Pointer to the Elliptic Curve Context that has been initialized.
              If the allocations fails, SlhNewByNid() returns NULL.
**/
VOID *
EFIAPI
SlhNewByNid (
  ...
  )
{
  ...
  PKeyCtx = EVP_PKEY_CTX_new_id (OpenSslNid, NULL);
  if (PKeyCtx == NULL) {
    return NULL;
  }
...
{
  EVP_PKEY_free ((EVP_PKEY *)SlhContext);
}

/**
  Gets the public key component from the established EC context.
  The Slh context should be correctly initialized by SlhNewByNid, and successfully
  ...
  )
{
  ...
  SlhKey = (EVP_PKEY *)SlhContext;
  if (!NidToKeySize (EVP_PKEY_id (SlhKey), FALSE, &KeySize)) {
    return FALSE;
  }
  ...
  *PublicKeySize = KeySize;
  if (EVP_PKEY_get_raw_public_key (SlhKey, PublicKey, PublicKeySize) != 1) {
    return FALSE;
  }
  ...
  )
{
  ...
  TempKey = EVP_PKEY_new_raw_public_key (EVP_PKEY_id (SlhKey), NULL, PublicKey, (UINT32)*PublicKeySize);
  if (TempKey == NULL) {
    return FALSE;
  }

  if (EVP_PKEY_copy_parameters (SlhKey, TempKey) != 1) {
  ...
  )
{
  ...
  TempKey = EVP_PKEY_new_raw_private_key (EVP_PKEY_id (SlhKey), NULL, PrivateKey, (UINT32)*PrivateKeySize);
  if (TempKey == NULL) {
    return FALSE;
  }
  ...
  )
{
  ...
  SlhKey = (EVP_PKEY *)SlhContext;
  if (!NidToSigSize (EVP_PKEY_id (SlhKey), &SigSizeRequired)) {
    return FALSE;
  }
  ...
  *SigSize = SigSizeRequired;
  ...
  if (EVP_DigestSignInit (MdCtx, NULL, NULL, NULL, SlhKey) != 1) {
    goto Exit;
  }
  ...
  )
{
  ...
  if (EVP_DigestVerifyInit (MdCtx, NULL, NULL, NULL, SlhKey) != 1) {
    goto Exit;
  }
